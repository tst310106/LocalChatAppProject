...
type MessageObjList = {
    messageList: MessageObj[]; // リストとして表示するメッセージ
    updateTime: string; // メッセージのリストの更新時間の文字列（YYYY-MM-DD HH:mm:ss）
}

// ローカルストレージから、現在の最新のメッセージリストのデータを取得する。

// Store中のstateの初期値initMesasgeListを、以下の条件に従って設定する。
// ◆最新のメッセージリストがローカルストレージ上に存在する場合
// 　ローカルストレージの値を各プロパティに代入する。
// ◆最新のメッセージリストがローカルストレージ上に存在しない場合
// 　messageList： []
// 　updateTime: ''
const initMesasgeList: MessageObjList;

// ディスパッチ時に送信されたアクションを元に、storeのstateの値を変更し、表示に反映させるレデューサー
// 第1引数：store中のstate
// 第2引数：ディスパッチ時に送信されたアクション
// 戻り値：新たにstoreに保管するstate
export function chatReducer(state: MessageObjList = initMesasgeList, action: SendMessageAction | ReloadListAction): MessageObjList{
...
// 第2引数：ディスパッチ時に送信されたアクション
// 戻り値：新たにstoreに保管するstate
function sendMessageReduce(state: MessageObjList, action: ReloadListAction): MessageObjList {
...
// 第1引数：store中のstate
// 戻り値：新たにstoreに保管するstate
function reloadListReduce(state: MessageObjList): MessageObjList  {
...
type SendMessageAction = {
    ...
    messageObj: MessageObj; // メッセージの情報
}

// 送信されたメッセージを表示するためのアクションを生成する。
// 第1引数：送信されたメッセージの情報
// 戻り値：第一引数を元に生成されたアクション
export function sendMessage(messageObj: MessageObj): SendMessageAction{
